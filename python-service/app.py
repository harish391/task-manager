...
@app.route('/api/python/analytics/tasks', methods=['GET'])
...
@app.route('/api/python/process/task', methods=['POST'])
def process_task():
    try:
        data = request.json
        task = data.get('task', {})
        processed_task = {
            ...
            'estimated_hours': _calculate_estimate(task.get('priority', 'medium')),
        }
        return jsonify(processed_task)
    except Exception as e:
        return jsonify({'error': str(e)}), 400

def _calculate_estimate(priority):
    estimates = {'low': 1, 'medium': 2, 'high': 4}
